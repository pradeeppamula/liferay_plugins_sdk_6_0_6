...
import org.ieeecs.communities.util.CSHeroContentUtil;
...
public class CSHeroContentController extends BaseController implements ResourceAwareController {
    static final Logger LOGGER = Logger.getLogger(CSHeroContentController.class);
	...
	protected ModelAndView handleRenderRequestInternal(RenderRequest renderRequest, RenderResponse renderResponse) throws Exception {
		...
		try {
            ...
            CSHeroContentUtil.putPortletPreferencesIntoModel(prefs, model);
			...
			String modifiedByUserId = prefs.getValue("modifiedByUserId", CSHeroContentUtil.USERID);
			String currentMode = prefs.getValue("portletMode", CSHeroContentUtil.MODE);
            // set the group id
            groupId = Long.toString(themeDisplay.getScopeGroupId());
			if ( currentUserId.trim().equalsIgnoreCase(modifiedByUserId.trim()) && currentMode.trim().equalsIgnoreCase(CSHeroContentUtil.MODE) ) {
    ...
    public ModelAndView handleResourceRequest(ResourceRequest request,
        ...
        try{
            ...
            if (CSHeroContentUtil.SAVE_CONFIG.equalsIgnoreCase(requestType)) {
                // update the data
                if(CSHeroContentUtil.updatePortletData(request, modifiedByUserId)) {
