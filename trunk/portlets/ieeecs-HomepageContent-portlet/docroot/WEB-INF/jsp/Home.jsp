...
<style type="text/css">
	...
	<script type="text/x-handlebars" data-template-name="content">
            {{#if isLoading}}
                <div class="content-loading-container container">
                    <i class="icon-spinner icon-spin icon-large"></i> Loading
                </div>
            {{else}}
                {{#if isArticle}}
                     {{view ContentApp.ArticleView}}
                {{else}}
                    {{#if isWebinar}}
                        {{view ContentApp.WebinarView}}
                    {{else}}
                        <div class="alert-warning-container alert alert-block alert-warning fade in warning">
                            <h2>
                                <i class="icon-warning-sign icon-2x icon-fixed-width"></i>
                                Content Not Found.
                            </h2>
                            <p>Please verify that you selected the correct item.  If you are still having trouble contact <strong>help@computer.org</strong> for help.</p>
                        </div> <!-- /.warning -->
                    {{/if}}
                {{/if}}
		    {{/if}}
   ...
   <script type="text/x-handlebars" data-template-name="webinar">
        <div class="container">
            <div class="row">
        ...
        <div class='content-body'>
            {{#if hasFullAccess}}
                ...
                <br />
                <div class="webinar-flowplayer-container"></div>
                <div id="webinar-not-found-${id}" class="alert-warning-container alert alert-block alert-warning fade hide warning">
                    <h2>
                        <i class="icon-warning-sign icon-2x icon-fixed-width"></i>
                        Webinar Not Found.
                    </h2>
            ...
            {{else}}
                ...
                <br /><br />
                <a href="/portal/web/myhome/webinar-bundle" class="btn btn-medium btn-block btn-primary">Learn More About Our Webinars</a>
            {{/if}}
        </div><!-- /.content-body -->
    </script>
   ...
   <script type="text/x-handlebars" data-template-name="article">
        ...
        <div class='content-body'>
              <div class="alerts-container container">
                <div class="alert-success-container alert alert-block alert-success fade hide success">
                    <button type="button" class="close" aria-hidden="true" {{ action 'hideAddBundleSuccess' }}>x</button>
                    <h4><i class="icon-check-sign icon-3x icon-fixed-width"></i>Added To Bundle!</h4>
                </div> <!-- /.success -->
                <div class="alert-danger-container alert alert-block alert-danger fade hide error">
                    <button type="button" class="close" aria-hidden="true" {{ action 'hideAddBundleError' }}>x</button>
                    {{#if bundleLimitError}}
                        <p>
                            <i class="icon-warning-sign icon-3x icon-fixed-width"></i>
                            Your bundle is at its limit, would you like to increase the size?
                            <a class="btn btn-default" href="#" {{ action 'goToBundlePurchase' }}>Make it happen</a>
                        </p>
                    {{else}}
                ...
                <div class="alert-warning-container alert alert-block alert-warning fade hide warning">
                    <button type="button" class="close" aria-hidden="true"  {{ action 'hideAddBundleConfirm' }}>x</button>
                    <p>
                        <i class="icon-warning-sign icon-3x icon-fixed-width"></i>
                        Are you sure you would like to add this item to your bundle?
                        <button {{bindAttr class=":btn :btn-default isSavingToBundle:disabled"}} {{ action 'addItemToBundle' }}>
                             {{#unless isSavingToBundle }}
                                Of Course
                            {{else}}
                                Please Wait
                            {{/unless}}
            ...
            {{#if hasFullAccess}}
                {{{summary}}}
            {{else}}
                 <div class="add-to-bundle-container">
                    <a href="#" class="btn btn-info btn-xs"  {{ action 'showAddBundleConfirm' }}>Add To Bundle</a>
                </div>
                {{{summary}}}
                 <br /><br />
                <a href="/portal/web/myhome/article-bundle" class="btn btn-medium btn-block btn-primary">Learn More About Our Article Bundles</a>
            {{/if}}
        </div> <!-- /.content-body -->
    </script>
